...
module.exports = function(options) {
  ...
  function get(key, options) {
    ...
    logger.log(`get - cacher id: ${self.id}, key: ${key}`, { value });
    return value;
  }
  ...
  async function getAndSet(key, getter, options) {
    ...
    if (!isFunction(getter)) {
      logger.log(`getAndSet - no getter function is defined - cacher id: ${self.id}, key: ${key}`, { options });
      return undefined;
    }
    ...
    if (!value) {
      logger.log(`getAndSet - retrieving value from generator - cacher id: ${self.id}, key: ${key}`, { options });
      value = await getter();

      set(key, value, options);
    }

    logger.log(`getAndSet - cacher id: ${self.id}, key: ${key}`, { value });
    return value;
  }
  ...
  function set(key, value, options) {
    ...
    if (!self.storeUndefinedObjects && (value === undefined || value === null || (typeof value.isNull === "function" && value.isNull()))) {
      logger.log(`set - not storing value as it is not defined - cacher id: ${self.id}, key: ${key}`, { value, options });
      remove(key, options);
      raiseEvent(self.removedCallback, key);
      return;
    }
    ...
    logger.log(`set - stored value - cacher id: ${self.id}, key: ${key}`, { cachedData: cacheValue });
  ...
  function keys() {
    ...
    logger.log(`keys - cacher id: ${self.id}`, { keys });
    return keys;
  }
